...
export class ServiceWorkerStore implements AsyncStorage {
    ...
    private _configurationName = "default";
    ...
    public async clear(): Promise<void>{ 
        await sendMessage(getRegistration())({ type: 'clear', data: {  }, configurationName: this._configurationName });
    }
    /** Returns the current value associated with the given key, or null if the given key does not exist. */
    public async getItem(key: string): Promise<string | null>{ 
        const result = await sendMessage(getRegistration())({ type: 'getItem', data: { key: key },configurationName: this._configurationName });
        return result.item;
    }
    /** Returns the name of the nth key, or null if n is greater than or equal to the number of key/value pairs. */
    public async key(index: number): Promise<string | null>{ 
        const result = await sendMessage(getRegistration())({ type: 'key', data: { index: index },configurationName: this._configurationName });
        return result.key;
    }
    /**
    ...
    public async removeItem(key: string): Promise<void>{ 
        await sendMessage(getRegistration())({ type: 'removeItem', data: { key: key },configurationName: this._configurationName });
    }
    /**
    ...
    public async setItem(key: string, value: string): Promise<void>{ 
        await sendMessage(getRegistration())({ type: 'setItem', data: { key: key , value: value},configurationName: this._configurationName });
    }
...
export const startWorkerAsync = async(serviceWorkerRelativeUrl, configurationName) => {
...
const initAsync = async () => {
    const result = await sendMessage(_sWRegistration)({
        type: 'init',
        data: {},
...
await startWorkerAsync('/ServiceWorker.js', 'default');
initAsync();
